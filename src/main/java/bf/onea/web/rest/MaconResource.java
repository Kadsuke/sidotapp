...
/**
 * REST controller for managing {@link bf.onea.domain.Macon}.
 */
@RestController
@RequestMapping("/api")
public class MaconResource {
    ...
    private static final String ENTITY_NAME = "macon";
    ...
    public MaconResource(MaconService maconService) {
     ...
     */
    @PostMapping("/macons")
    public ResponseEntity<MaconDTO> createMacon(@RequestBody MaconDTO maconDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/macons/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /macons} : Updates an existing macon.
     *
     ...
     */
    @PutMapping("/macons")
    public ResponseEntity<MaconDTO> updateMacon(@RequestBody MaconDTO maconDTO) throws URISyntaxException {
        log.debug("REST request to update Macon : {}", maconDTO);
        if (maconDTO.getId() == null) {
